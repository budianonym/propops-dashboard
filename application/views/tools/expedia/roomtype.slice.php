					...
					<div class="form-group">
						...
						<div class="container-fluid table-responsive" id="export1">
							<table class="table-striped" >
								...
								$i = 1;
								foreach ($ids as $id) {;?>
								<?php $profile = http_request('https://EQC_Redawning:EWK3E7KDY4V2iR7@services.expediapartnercentral.com/properties/'.$id.'/roomTypes');
								...
								<tr>
                                    ...
                                    if(!array_key_exists("errors",$profile)){
                                        $res = $profile["entity"];
                                        // echo count($profile["entity"]);
                                        // var_dump($profile["entity"][0]["resourceId"]);
                                        foreach($res as $key){
                                            echo $key["resourceId"];
                                            if($a != count($profile["entity"])){
                                                echo "<br>";
                                            }
    $a++;
                                        }
